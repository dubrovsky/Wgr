...
public class EDIConvertor extends Convertor {

  final static private Logger log = LoggerFactory.getLogger(EDIConvertor.class);
  ...
  protected static Pattern prin_p = Pattern.compile("^[a-zA-Z]{4}");
  protected static Pattern nkon_p = Pattern.compile("[0-9]{7,9}$");

  protected static Pattern prinnkon_p = Pattern.compile("^[a-zA-Z]{4}[0-9]{7,9}$");
  protected static final SimpleDateFormat dateTimeFormater1 = new SimpleDateFormat("dd.MM.yyyy");
  protected static final SimpleDateFormat dateTimeFormater2 = new SimpleDateFormat("dd.MM.yy");
  protected static final SimpleDateFormat dateTimeFormater3 = new SimpleDateFormat("dd/MM/yyyy");
  protected static final SimpleDateFormat dateTimeFormater4 = new SimpleDateFormat("dd/MM/yy");
  ...
  private String recipient = "IRC.RW.BY";
  private final String folder = "EDI_Failed";
  private EdiDir ediDir;
  ...
  public EDIConvertor(String script, EdiDir ediDir) {
    this.ediDir = ediDir;
    readCfg(script);
  }
...
//  }
  ...
  private void readCfg(String script) {
    try {
  ...
  public void sendIftmin(Long csId) throws Exception {
    ...
    try {
      ...
      if (cs != null) {
        ...
        String query = "UPDATE CimSmgs cs SET cs.iftminOut" + ediDir.getSuffix() + "=:a, cs.iftminId" + ediDir.getSuffix() + "=:b, cs." + ediDir.statusCol + "=:s WHERE cs.hid=:id";
...
//    }
  ...
  protected static String format(Object str, int len) {
    return Utils.format(str, len);
  }

  protected static String format(Object str, int len, int count) {
    return Utils.format(str, len, count);
  }

  protected static String ge(String[] ar, int idx) {
    return Utils.ge(ar, idx);
  }

  protected static String[] split(String input, String regex, String meta) {
    return Utils.split(input, regex, meta);
  }

  protected static String[] split(String input, String regex) {
  ...
  protected String getText(CimSmgs smgsOb, long messageId, Date curDate, List<CimSmgsCarList> carList) throws Exception {
    String text;
    text =  "UNB+UNOA:1+TRCONT.RU::HOST+" + format(recipient, 35) + "::HOST+" +
...
//
    ...
    for (CimSmgsDocs csdOb : csd9.values()) {
      d = csdOb.getDat();
      text += "DOC+" + format(csdOb.getCode(), 3) + "+" +format(csdOb.getText(), 35) + ":2:" + format(d != null ? dateTimeFormater1.format(d) : "", 35)  + nl;
    }
    for (CimSmgsInvoice invOb : smgsOb.getPackDoc().getCsInvoices()) {
      text += "DOC+380+" +format(invOb.getInvoice(), 35) + ":4" + nl;
    }
...
//    }
    ...
    if (StringUtils.isNotBlank(g29)) {
      String[] g29Array = Utils.split(g29, " ");
      text += "LOC+13+" + format(Utils.ge(g29Array, 0), 2) + format(Utils.ge(g29Array, 1), 23) + ":37:288:" +
              format(Utils.ge(g29Array, 2), 70) + nl;
    }
...
//    text += "RFF+CT:" + format(invOb.getN_dog(), 35) + nl;
    ...
    for (CimSmgsInvoiceGruz grOb : invOb.getInvoiceGruzs().values()) {
      ...
      text += "QTY+47:" + format(grOb.getKole(), 15) + ":" + format(grOb.getEizm(), 3) + nl;
  ...
  private void saveLog(Session session, long id, String mesName, String text, Long hid_src) throws HibernateException {
    BIftminLog bean = new BIftminLog();
    bean.setHid(String.valueOf(id));
    bean.setOut_text(text);
    bean.setSrc("CIMSMGS");
    bean.setMes_name(mesName);
    bean.setHid_src(hid_src);
    bean.setDir(ediDir.getDirName());
    session.save(bean);
  }

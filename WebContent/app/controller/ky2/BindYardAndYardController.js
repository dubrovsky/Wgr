Ext.define('TK.controller.ky2.BindYardAndYardController', {
    ...
    views: [
        'ky2.yard.Yard2YardBindTreeForm'
    ...
    stores: [
        ...
        'ky2.YardBindTreeNodesLeft',
        'ky2.YardBindTreeNodesRight'
    ...
    init: function () {
        ...
        this.control({
            ...
            // },
            'ky2yardlist button[action="getYardAndYardForBind"]': {
                click: this.getYardAndYardForBind
            },
            // 'ky2poezdoutlist button[action="getPoesdAndYardForBind"]': {
            //     click: this.getPoesdOutAndYardForBind
            // },
            'ky2yard2yardbindtreeform treepanel#treepanelLeft': {
                selectionchange: this.selectionchangePoezd
            },
            'ky2yard2yardbindtreeform treepanel#treepanelRight': {
            ...
            //     selectionchange: this.selectionchangeYard
            // },
            'ky2yard2yardbindtreeform treepanel#treepanelLeft > treeview': {
                drop: this.dropToYard,
                nodedragover: this.beforeDropToYard,
                beforedrop: this.beforeDropDataToYard
            },
            'ky2yard2yardbindtreeform treepanel#treepanelRight > treeview': {
                drop: this.dropToYard,
                nodedragover: this.beforeDropToYard,
                beforedrop: this.beforeDropDataToYard
            },
            ...
            // },
            'ky2yard2yardbindtreeform treepanel#treepanelRight button[action="clTrAvtoFilter"]': {
                click: this.getController('ky2.BindPoezdAndYardController').buildClTrAvtoFilter
            },
            'ky2yard2yardbindtreeform treepanel#treepanelRight button[action="clearFiltr"]': {
            ...
            //     click: this.moveAllNodesRight
            // },
            'ky2yard2yardbindtreeform button[action=save]': {
                click: this.bindYardAndYard
            },
            // 'ky2poezd2yardbindtreeformout button[action=save]': {
            //     click: this.bindPoezdAndYard
            // },
            'ky2yard2yardbindtreeform button[action=saveExit]': {
                click: this.bindYardAndYardAndExit
            },
            ...
            // },
            'ky2yard2yardbindtreeform button[action="upload"]': {
                click: this.getController('ky2.BindPoezdAndYardController').onUploadXLS
            }
    ...
    // getYardAndYardForBindCheck: function () {
    //     var yardlist = this.getYardlist();
    //     if (!TK.Utils.isRowSelected(yardlist)) {
    //         return false;
    //     }
    //     this.getYardAndYardForBind('ky2yard2yardbindtreeform', yardlist.getSelectionModel().getLastSelected().get('hid'), null);
    // },

    getYardAndYardForBind: function () {
        this.getCenter().setLoading(true);
        Ext.Ajax.request({
            url: 'ky2/secure/BindYardAndYard.do',
            params: {
                action: 'get_yard_and_yard_for_bind',
            ...
            callback: function (options, success, response) {
                if (success) {
                    ...
                    var yardSectorArr = respObj['rows'];

                    this.getCenter().remove(this.getCenter().getComponent(0), true);
                    var bindcontainer = Ext.widget('ky2yard2yardbindtreeform', {title: 'Контейнерная площадка'});
                    ...
                    this.getTreepanelLeft().setTitle(this.getController('ky2.BindPoezdAndYardController').titleForYard('Контейнерная площадка' + '<br/>' + '<br/>'));
                    var rootNode = this.getTreepanelLeft().getRootNode();
                    if (yardSectorArr && yardSectorArr.length > 0) {
                        this.initYardSectorNodes(yardSectorArr, rootNode);
                    }
                    this.getTreepanelRight().setTitle(this.getController('ky2.BindPoezdAndYardController').titleForYard('Контейнерная площадка' + '<br/>' + '<br/>'));
    ...
    checkBeforeMoveToYard: function (records, targetModel, optParams) {
        ...
        for (var i = 0; i < records.length; i++) {
            ...
            if (isDrop) {
                ...
                isDrop = sourceParentModel.get('who') === 'yard'; // can be moved in same yard
            }
            ...
            if (isDrop) {
                isDrop = sourceParentModel.get('yardSectorHid') !== targetModel.get('hid'); // prevent dropping in same node
            }
            if (isDrop) {
                var targetParentNode = targetModel.parentNode;
                while (targetParentNode.parentNode != null)
                    targetParentNode = targetParentNode.parentNode;
                isDrop = sourceParentModel.store.storeId !== targetParentNode.store.storeId; // prevent dropping in same tree
            }
            ...
            // } else { // same tree
                ...
                for (var y = 0; y < this.sourceYardModels.length; y++) { // // this.sourceYardModels here yardsectors
                    ...
                    if (records[i].get('yardHid') === this.sourceYardModels[y].get('hid')) {
                        ...
                        // clear yards on other panel
                        var dublicateFreeYard = targetModel.parentNode.findChild('hid', this.sourceYardModels[i].parentNode.get('hid')).findChild('hid', this.sourceYardModels[i].get('hid'));
                        dublicateFreeYard.removeAll();
                        dublicateFreeYard.set('konts', []);
                        dublicateFreeYard.set('updated', true);
                        dublicateFreeYard.parentNode.set('updated', true);
                        dublicateFreeYard.parentNode.set('contsInYardSector', (dublicateFreeYard.parentNode.get('contsInYardSector') - 1));
                        this.updateYardSectorModelText(dublicateFreeYard.parentNode);
                        // add yard on other panel
                        var sectorForAddKonts = this.sourceYardModels[y].parentNode.parentNode.findChild('hid', targetModel.get('hid'));
                        targetModel.eachChild(function (yard) {
                            var kont4Add = yard.findChild('hid', records[y].get('hid'));
                            if (kont4Add != null) {
                                var yard4AddHid = kont4Add.parentNode.get('hid'),
                                    yard4Add = sectorForAddKonts.findChild('hid', yard4AddHid);
                                kont4Add = kont4Add.copy(null, true);
                                yard4Add.appendChild(kont4Add);
                                yard4Add.set('updated', true);
                                yard4Add.set('konts', kont4Add.data);
                                sectorForAddKonts.set('updated', true);
                                sectorForAddKonts.set('contsInYardSector', (sectorForAddKonts.get('contsInYardSector') + 1));
                                this.updateYardSectorModelText(sectorForAddKonts);
    ...
    bindYardAndYardAndExit: function () {
        this.bindYardAndYard(1);
    },

    bindYardAndYard: function (close) {
        console.log('save yard bindYardAndYard');
        ...
        var url = 'ky2/secure/BindYardAndYard.do';
        this.getCenter().setLoading(true);
        Ext.Ajax.request({
            url: url,
            params: {
